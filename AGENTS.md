...
```
journal/        = source code    (your conversations - the raw material)
...
### Layer 1: `journal/` - Conversation Logs (Immutable Source)

Daily logs capture what happened in your AI coding sessions. These are the "raw sources" - append-only, never edited after the fact. Logs are organized by year and month.
...
|---------|---------|---------------|---------|
| [[concepts/supabase-auth]] | Row-level security patterns and JWT gotchas | journal/2026/04/2026-04-02.md | 2026-04-02 |
| [[connections/auth-and-webhooks]] | Token verification patterns shared across Supabase auth and Stripe webhooks | journal/2026/04/2026-04-02.md, journal/2026/04/2026-04-04.md | 2026-04-04 |
```
...
- Source: journal/2026/04/2026-04-01.md
...
tags: [domain, topic]
sources:
  - "journal/2026/04/2026-04-01.md"
  - "journal/2026/04/2026-04-03.md"
created: 2026-04-01
updated: 2026-04-03
---
...
- [[journal/2026/04/2026-04-01.md]] - Initial discovery during project setup
- [[journal/2026/04/2026-04-03.md]] - Updated after debugging session
```
...
- Every article must link back to its source journal logs
...
- **Sources:** Always link back to the journal log(s) that contributed to an article
...
|-- claude_memory_system/                  # All memory system files live here
|   |-- AGENTS.md                          #   This file - schema + full technical reference
|   |-- pyproject.toml                     #   Dependencies (uv --project points here)
|   |-- uv.lock                            #   Locked dependencies
|   |-- journal/                            #   "Source code" - conversation logs (YYYY/MM/ hierarchy)
...
|   |   |-- qa/                            #     Filed query answers (compounding knowledge)
|   |-- scripts/                           #   CLI tools
|   |   |-- compile.py                     #     Compile journal logs -> knowledge articles
...
|   |   |-- session-end.py                 #     Extracts conversation -> journal log
...
Commands use `--project claude_memory_system` so `uv` finds the `pyproject.toml` inside that directory. Paths to hook scripts are relative to the workspace root. Empty `matcher` catches all events.

### Hook Details

**`session-start.py`** (SessionStart)
- Pure local I/O, no API calls, runs in under 1 second
- Reads `knowledge/index.md` and the most recent journal log
...
6. Appends result to `journal/YYYY/MM/YYYY-MM-DD.md`
7. Cleans up temp context file
8. **End-of-day auto-compilation:** If it's past 6 PM local time (`COMPILE_AFTER_HOUR = 18`) and today's journal log has changed since its last compilation (hash comparison against `state.json`), spawns `compile.py` as another detached background process. This means compilation happens automatically once a day without needing a cron job or manual trigger.
...
- Builds a prompt with: AGENTS.md schema, current index, all existing articles, and the journal log
- Claude reads the journal log, decides what concepts to extract, and writes files directly
- `permission_mode="acceptEdits"` auto-approves all file operations
- Incremental: tracks SHA-256 hashes of journal logs in `state.json`, skips unchanged files
- Cost: ~$0.45-0.65 per journal log (increases as KB grows)

**CLI:**
```bash
uv run --project claude_memory_system python claude_memory_system/scripts/compile.py              # compile new/changed only
uv run --project claude_memory_system python claude_memory_system/scripts/compile.py --all        # force recompile everything
...
**CLI:**
```bash
uv run --project claude_memory_system python claude_memory_system/scripts/lint.py                    # all checks
uv run --project claude_memory_system python claude_memory_system/scripts/lint.py --structural-only  # skip LLM check (free)
```
...
`claude_memory_system/scripts/state.json` tracks:
- `ingested` - map of journal log filenames to SHA-256 hashes, compilation timestamps, and costs
- `query_count` - total queries run
- `last_lint` - timestamp of most recent lint
- `total_cost` - cumulative API cost

`claude_memory_system/scripts/last-flush.json` tracks flush deduplication (session_id + timestamp).
...
`pyproject.toml` (in `claude_memory_system/`):
