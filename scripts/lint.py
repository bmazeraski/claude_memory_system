...
from config import KNOWLEDGE_DIR, JOURNAL_DIR, REPORTS_DIR, now_iso, today_iso
from utils import (
    count_inbound_links,
    extract_wikilinks,
    file_hash,
    ...
    for log_path in list_raw_files():
        if log_path.name not in ingested:
            issues.append({
                "severity": "warning",
                "check": "orphan_source",
                "file": str(log_path.relative_to(JOURNAL_DIR.parent)).replace("\\", "/"),
                "detail": f"Uncompiled journal log: {log_path.name} has not been ingested",
            })
    return issues
