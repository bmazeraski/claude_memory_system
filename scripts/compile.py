...
Usage:
    ...
    uv run python compile.py --file journal/2026/04/2026-04-01.md  # compile a specific log
...
from config import AGENTS_FILE, CONCEPTS_DIR, CONNECTIONS_DIR, JOURNAL_DIR, KNOWLEDGE_DIR, journal_path, now_iso
from utils import (
    file_hash,
    list_raw_files,
    list_wiki_articles,
...
6. **Append to knowledge/log.md** - Add a timestamped entry:
   ...
   - Source: {str(log_path.relative_to(ROOT_DIR)).replace(chr(92), '/')}
...
def main():
    ...
    if args.file:
        ...
        if not target.is_absolute():
            # Try relative to project root first, then parse date from filename
            target = ROOT_DIR / args.file
        if not target.exists():
            from datetime import date
            try:
                d = date.fromisoformat(Path(args.file).stem)
                target = journal_path(d)
            except ValueError:
                pass
        if not target.exists():
    ...
    if not to_compile:
        print("Nothing to compile - all journal logs are up to date.")
        return
