...
this hook reads the knowledge base index and recent journal log, then injects
...
def get_recent_log() -> str:
    """Read the most recent journal log (today or yesterday)."""
    today = datetime.now(timezone.utc).astimezone()

    for offset in range(2):
        date = today - timedelta(days=offset)
        log_path = JOURNAL_DIR / f"{date.year}" / f"{date.month:02d}" / f"{date.strftime('%Y-%m-%d')}.md"
        if log_path.exists():
